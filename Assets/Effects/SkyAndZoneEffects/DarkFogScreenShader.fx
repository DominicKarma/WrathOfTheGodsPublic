...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float sourceDistanceWarp = tex2D(fogNoiseTexture, worldAdjustedCoords * 3 - float2(0, uTime * 0.33));
    
    // Calculate noise values.
    noiseOffset.x += uTime * 0.08;
    float noise = lerp(tex2D(fogNoiseTexture, worldAdjustedCoords * 0.61 + noiseOffset).r, 0.5, 0.4);
