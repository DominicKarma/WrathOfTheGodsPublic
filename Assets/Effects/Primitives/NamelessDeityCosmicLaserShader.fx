...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float2 coords = input.TextureCoordinates + float2(-0.034, 0);
    ...
    float4 spaceColor = CalculateSpaceColor(cylindricalCoords) * pow(sin(coords.y * 3.141), 0.51);
    ...
    return lerp(finalColor, float4(backColor, 1), CalculateBackLineInterpolant(coords, input.TextureCoordinates - 0.5));
}
