...
namespace NoxusBoss.Content.NPCs.Bosses.NamelessDeity.Projectiles
{
    public class LightSlash : ModProjectile, IProjOwnedByBoss<NamelessDeityBoss>
    {
        ...
        public Color PrimitiveColorFunction(float completionRatio)
        {
            ...
            float distortionStrength = Remap(Time, 0f, 11f, 1f, 0.1f);
        ...
        public void DrawToTarget()
        {
            ...
            var slashShader = ShaderManager.GetShader("GenericTrailStreak");
            slashShader.SetTexture(StreakBloomLine, 1);

            // Initialize the slash drawer.
            SlashDrawer ??= new(PrimitiveWidthFunction, PrimitiveColorFunction, null, true, slashShader);
            ...
            for (int i = 0; i < 7; i++)
            {
                float interpolant = i / 6f * Remap(Time, 0f, 8f, 0.1f, 1f);
                slashPoints.Add(QuadraticBezier(start, middle, end, interpolant));
            }

            SlashDrawer.Draw(slashPoints, -Main.screenPosition, 14);
        }
    }
}
